...
class DatumComparatorTest {
    ...
    private val nullValues = listOf(
        EquivValues(
            ...
            Datum.nullValue(PType.typeIntArbitrary()), // `null.int`,
            Datum.nullValue(PType.typeStruct()) // `null.struct`
        )
    )
    ...
    private val nonNullDatum = listOf(
        ...
        EquivValues(
            ...
            Datum.decimalArbitrary(BigDecimal("-0.50000000000000000000000000")),
            Datum.real(-0.5e0f),
            Datum.doublePrecision(-0.5e0)
        ),
        EquivValues(
            Datum.decimalArbitrary(BigDecimal("-0.0")),
            Datum.decimalArbitrary(BigDecimal("-0.0000000000")),
            Datum.real(0e0f),
            Datum.doublePrecision(0e0),
            Datum.real(-0e0f),
            Datum.doublePrecision(-0e0),
            Datum.decimalArbitrary(BigDecimal("0e10000")),
            Datum.integer(0),
            Datum.integer(-0),
            Datum.bigInt(0),
            Datum.bigInt(-0)
        ...
        EquivValues(
            ...
            Datum.decimalArbitrary(BigDecimal("5e9")),
            Datum.decimalArbitrary(BigDecimal("5.00000e9")),
        ),
        EquivValues(
            // make sure there are at least two +inf
        ...
        EquivValues(
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, timeZone = TimeZone.UnknownTimeZone)),
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, nano = 0, timeZone = TimeZone.UnknownTimeZone)),
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, timeZone = TimeZone.UnknownTimeZone)),
            // time second precision handled by time constructor
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, timeZone = TimeZone.UtcOffset.of(0))),
        ),
        EquivValues(
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, nano = 100000000, timeZone = TimeZone.UnknownTimeZone)),
        ),
        EquivValues(
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, nano = 0, timeZone = TimeZone.UtcOffset.of(-8, 0))),
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, timeZone = TimeZone.UtcOffset.of(-8, 0))),
        ),
        EquivValues(
            Datum.timeWithoutTZ(time(hour = 12, minute = 12, second = 12, nano = 100000000, timeZone = TimeZone.UtcOffset.of(-9, 0))),
        ),
        EquivValues(
            Datum.timestampWithoutTZ(timestamp(year = 2017, timeZone = TimeZone.UtcOffset.of(0, 0))), // `2017T`
            Datum.timestampWithoutTZ(timestamp(year = 2017, month = 1, timeZone = TimeZone.UtcOffset.of(0, 0))), // `2017-01T`
            Datum.timestampWithoutTZ(timestamp(year = 2017, month = 1, day = 1, timeZone = TimeZone.UtcOffset.of(0, 0))), // `2017-01-01T`
            Datum.timestampWithoutTZ(timestamp(year = 2017, month = 1, day = 1, hour = 0, minute = 0, second = 0, timeZone = TimeZone.UtcOffset.of(0, 0))), // `2017-01-01T00:00-00:00`
            Datum.timestampWithoutTZ(timestamp(year = 2017, month = 1, day = 1, hour = 1, minute = 0, second = 0, timeZone = TimeZone.UtcOffset.of(1, 0))) // `2017-01-01T01:00+01:00`
        ),
        EquivValues(
            Datum.timestampWithoutTZ(timestamp(year = 2017, month = 1, day = 1, hour = 1, minute = 0, second = 0, timeZone = TimeZone.UtcOffset.of(0, 0))) // `2017-01-01T01:00Z`
        ),
        EquivValues(
            Datum.string(""),
...
//        ),
        EquivValues(
            ...
            Datum.sexp(listOf(Datum.decimalArbitrary(BigDecimal("1.0000000000000")))) // `(1.0000000000000)`
        ),
        EquivValues(
            Datum.sexp(listOf(Datum.timestampWithoutTZ(timestamp(year = 2012)), Datum.real(Float.NaN))) // `(2012T nan)`
        ),
        EquivValues(
            Datum.sexp(listOf(Datum.timestampWithoutTZ(timestamp(year = 2012)), Datum.integer(1), Datum.integer(2), Datum.integer(3))) // `(2012T 1 2 3)`
        ),
        EquivValues(
        ...
        EquivValues(
            ...
            struct( // {'c': false, 'b': 1.00000000e3}
                "c" to Datum.bool(false), "b" to Datum.decimalArbitrary(BigDecimal("1.00000000e3"))
            )
        ),
        EquivValues(
            struct( // {'c': false}
        ...
        EquivValues(
            ...
            // <<true, 1, 1.0, `1e0`, true>>
            Datum.bag(listOf(Datum.bool(true), Datum.integer(1), Datum.decimalArbitrary(BigDecimal("1.0")), Datum.real(1e0f), Datum.bool(true)))
        ),
        EquivValues( // <<1>>
