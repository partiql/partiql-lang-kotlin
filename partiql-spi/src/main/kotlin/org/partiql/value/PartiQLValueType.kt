...
public enum class PartiQLValueType {
    ...
    public fun toPType(): PType {
        return when (this) {
            DECIMAL, DECIMAL_ARBITRARY -> PType.typeDecimalArbitrary()
            INT8 -> PType.typeTinyInt()
            CHAR -> PType.typeChar(255)
            TIMESTAMP -> PType.typeTimestampWithTZ(6)
            DATE -> PType.typeDate()
            BOOL -> PType.typeBool()
            SYMBOL -> PType.typeSymbol()
            STRING -> PType.typeString()
            STRUCT -> PType.typeStruct()
            SEXP -> PType.typeSexp()
            LIST -> PType.typeList()
            BAG -> PType.typeBag()
            FLOAT32 -> PType.typeReal()
            INT -> PType.typeIntArbitrary()
            INT64 -> PType.typeBigInt()
            INT32 -> PType.typeInt()
            INT16 -> PType.typeSmallInt()
            TIME -> PType.typeTimeWithoutTZ(6)
            ANY -> PType.typeDynamic()
            FLOAT64 -> PType.typeDoublePrecision()
            CLOB -> PType.typeClob(Int.MAX_VALUE)
            BLOB -> PType.typeBlob(Int.MAX_VALUE)
            NULL, MISSING -> PType.typeUnknown()
            INTERVAL, BYTE, BINARY -> PType.typeDynamic() // TODO: REMOVE THIS
