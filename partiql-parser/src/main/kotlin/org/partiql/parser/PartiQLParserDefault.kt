...
import com.ibm.icu.text.UTF16.insert
import com.sun.org.apache.xalan.internal.lib.ExsltDatetime.date
import jdk.nashorn.internal.runtime.Debug
import org.antlr.v4.misc.Utils.select
 ...
 * but is slower than [PredictionMode.SLL]. Upon seeing a syntax error, this parser throws a [PartiQLParseException].
 */
internal class PartiQLParserDefault : PartiQLParser {

    @Throws(PartiQLParseException::class, InterruptedException::class)
    override fun parse(source: String): Statement {
        try {
            return PartiQLParserDefault.parse(source)
        } catch (throwable: Throwable) {
            throw PartiQLParseException.wrap(throwable)
        }
    }
    ...
    companion object {
         ...
         */
        private fun parse(input: String): Statement = try {
            parse(input, PredictionMode.SLL)
        } catch (ex: ParseCancellationException) {
            parse(input, PredictionMode.LL)
        }

        /**
         * Parses an input string [input] using the given prediction mode.
         */
        private fun parse(input: String, mode: PredictionMode): Statement {
            val tokens = createTokenStream(input)
            ...
            when (mode) {
                ...
                else -> throw PartiQLParseException("Unsupported parser mode: $mode")
            }
            val tree = parser.root()
            val visitor = Visitor(tokens.parameterIndexes)
            return visitor.visit(tree) as Statement
        }

        private fun createTokenStream(input: String): CountingTokenStream {
            val queryStream = input.byteInputStream(StandardCharsets.UTF_8)
            val inputStream = try {
    ...
    /**
     * Catches Lexical errors (unidentified tokens) and throws a [PartiQLParseException]
     */
    private class TokenizeErrorListener : BaseErrorListener() {
        @Throws(PartiQLParseException::class)
    ...
    /**
     * Catches Parser errors (malformed syntax) and throws a [PartiQLParseException]
     */
     ...
     */
    private class Visitor(private val parameterIndexes: Map<Int, Int> = mapOf()) : PartiQLBaseVisitor<AstNode>() {
        ...
        override fun visitQueryDql(ctx: GeneratedParser.QueryDqlContext) = visitDql(ctx.dql())

        override fun visitQueryDml(ctx: GeneratedParser.QueryDmlContext): AstNode = visit(ctx.dml())

        override fun visitRoot(ctx: GeneratedParser.RootContext) = when (ctx.EXPLAIN()) {
            null -> visit(ctx.statement()) as Statement
            else -> {
                var type: String? = null
                var format: String? = null
                val metadata = ctx.EXPLAIN().getSourceMetaContainer()
                ctx.explainOption().forEach { option ->
                    val parameter = try {
                        ExplainParameters.valueOf(option.param.text.toUpperCase())
                    } catch (ex: java.lang.IllegalArgumentException) {
                        throw option.param.error("Unknown EXPLAIN parameter.", ex)
                    }
                    when (parameter) {
        ...
        override fun visitSymbolPrimitive(ctx: GeneratedParser.SymbolPrimitiveContext): AstNode {
            // val metas = ctx.ident.getSourceMetaContainer()
            return when (ctx.ident.type) {
                ...
                else -> throw PartiQLParseException("Invalid symbol reference.")
            }
        }
        ...
        override fun visitQueryDdl(ctx: GeneratedParser.QueryDdlContext) = build {
            val op = visitDdl(ctx.ddl()) as DdlOp
            ddl(op, op.metas)
        }

        override fun visitDropTable(ctx: GeneratedParser.DropTableContext) = build {
            val id = visitSymbolPrimitive(ctx.tableName().symbolPrimitive())
            dropTable(id.toIdentifier(), ctx.DROP().getSourceMetaContainer())
        }

        override fun visitDropIndex(ctx: GeneratedParser.DropIndexContext) = build {
        ...
        override fun visitCreateTable(ctx: GeneratedParser.CreateTableContext) = build {
        ...
        override fun visitCreateIndex(ctx: GeneratedParser.CreateIndexContext) = build {
        ...
        override fun visitTableDef(ctx: GeneratedParser.TableDefContext) = build {
            val parts = visitOrEmpty(ctx.tableDefPart(), TableDefPart::class)
            tableDef(parts)
        }

        override fun visitColumnDeclaration(ctx: GeneratedParser.ColumnDeclarationContext) = build {
        ...
        override fun visitColumnConstraint(ctx: GeneratedParser.ColumnConstraintContext) = build {
            val name = ctx.columnConstraintName()?.let { visitSymbolPrimitive(it.symbolPrimitive()).name.text }
            val def = visit(ctx.columnConstraintDef(), ColumnConstraintDef::class)
            columnConstraint(name, def)
        }

        override fun visitColConstrNotNull(ctx: GeneratedParser.ColConstrNotNullContext) = build {
            columnNotnull()
        }

        override fun visitColConstrNull(ctx: GeneratedParser.ColConstrNullContext) = build {
            columnNull()
        }
        ...
        override fun visitDmlBaseWrapper(ctx: GeneratedParser.DmlBaseWrapperContext) = build {
            val sourceContext = when {
                ...
                else -> throw PartiQLParseException(
        ...
        private fun getCommandList(command: AstNode): List<DmlOp> {
            return when (command) {
                ...
                else -> throw PartiQLParseException("Unable to grab DML operation.", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitSelectorBasic(ctx: GeneratedParser.SelectorBasicContext) = build {
            ...
            when (ctx.mod.type) {
                ...
                else -> throw PartiQLParseException("Unsupported match selector.", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitEdgeAbbrev(ctx: GeneratedParser.EdgeAbbrevContext) = build {
            when {
                ...
                else -> throw PartiQLParseException("Unsupported edge type", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitPatternQuantifier(ctx: GeneratedParser.PatternQuantifierContext) = build {
            when {
                ...
                else -> throw PartiQLParseException("Unsupported quantifier", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitPatternRestrictor(ctx: GeneratedParser.PatternRestrictorContext) = build {
            ...
            when (ctx.restrictor.text.toLowerCase()) {
                ...
                else -> throw PartiQLParseException("Unrecognized pattern restrictor", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitParameter(ctx: GeneratedParser.ParameterContext) = build {
            val parameterIndex = parameterIndexes[ctx.QUESTION_MARK().symbol.tokenIndex]
                ?: throw PartiQLParseException("Unable to find index of parameter.", ErrorCode.PARSE_INVALID_QUERY)
        ...
        override fun visitSequenceConstructor(ctx: GeneratedParser.SequenceConstructorContext) = build {
            ...
            when (ctx.datatype.type) {
                ...
                else -> throw PartiQLParseException("Unknown sequence", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitLiteralIon(ctx: GeneratedParser.LiteralIonContext) = build {
            val ionValue = try {
                ...
                throw PartiQLParseException("Unable to parse Ion value.", ErrorCode.PARSE_UNEXPECTED_TOKEN, cause = e)
            }
            lit(
                ionValue,
        ...
        override fun visitTypeAtomic(ctx: GeneratedParser.TypeAtomicContext) = build {
            ...
            when (ctx.datatype.type) {
                ...
                else -> throw PartiQLParseException("Unsupported type.", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        override fun visitTypeCustom(ctx: GeneratedParser.TypeCustomContext) = build {
            ...
            val customName: String = when (val name = ctx.symbolPrimitive().getString().toLowerCase()) {
                ...
                else -> throw PartiQLParseException("Invalid custom type name: $name", ErrorCode.PARSE_INVALID_QUERY)
            }
        ...
        private fun visitBinaryOperation(
            ...
            when (op!!.type) {
                ...
                else -> throw PartiQLParseException("Unknown binary operator", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
        ...
        private fun visitUnaryOperation(operand: ParserRuleContext?, op: Token?, parent: ParserRuleContext? = null) =
            build {
                ...
                when (op!!.type) {
                    ...
                    else -> throw PartiQLParseException("Unknown unary operator", ErrorCode.PARSE_INVALID_QUERY)
                }
            }
        ...
        private fun Expr.getStringValue(token: Token? = null): String = when (this) {
            is Expr.Id -> this.name.text.toLowerCase()
            is Expr.Lit -> {
                when (this.value) {
        ...
        private fun Expr.Id?.toPigSymbolPrimitive(): SymbolPrimitive? = when (this) {
            null -> null
            else -> this.name.copy(metas = this.metas)
        }

        private fun Expr.Id.toIdentifier(): Identifier {
        ...
        private fun convertSymbolPrimitive(sym: GeneratedParser.SymbolPrimitiveContext?): SymbolPrimitive? =
            when (sym) {
                null -> null
        ...
        private fun GeneratedParser.SelectClauseContext.getMetas(): MetaContainer = when (this) {
            ...
            else -> throw PartiQLParseException("Unknown meta location.", ErrorCode.PARSE_INVALID_QUERY)
        }
        ...
        private fun GeneratedParser.SymbolPrimitiveContext.getString(): String {
            return when {
                ...
                else -> throw PartiQLParseException("Unable to get symbol's text.", ErrorCode.PARSE_INVALID_QUERY)
            }
        }
...
internal fun Token.exception(
    message: String,
    cause: Throwable? = null,
    context: MutableMap<String, Any> = mutableMapOf(),
): PartiQLParseException  {
    context["line_no"] = this.line.toLong()
    context["column_no"] = this.charPositionInLine.toLong() + 1
    context["token_description"] = this.type.getAntlrDisplayString()
    context["token_value"] = this.asIonElement()
    return PartiQLParseException(message, cause, context)
}
