...
abstract class SqlDialect : AstBaseVisitor<SqlBlock, SqlBlock>() {
    ...
    companion object {
        ...
        @JvmStatic
        val STANDARD = object : SqlDialect() {}
    }

    /**
     * Default entry-point, can also be us.
     */
    fun transform(node: AstNode): SqlBlock {
        val head = SqlBlock.nil()
        val tail = head
    ...
    override fun visitExprSFWSetOp(node: Expr.SFW.SetOp, tail: SqlBlock): SqlBlock {
        ...
        t = t concat SqlBlock.Nest(
            prefix = " (",
            postfix = ")",
            child = SqlBlock.nil().apply { visitExprSFW(node.operand, this) },
        )
        return t
    }
