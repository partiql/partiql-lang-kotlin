...
class JacksonPoem(symbols: Symbols) : Poem(symbols) {
    ...
    override fun apply(node: NodeSpec.Product) {
        ...
        node.builder.addProperty(
            PropertySpec.builder(cairn, String::class)
                .addModifiers(KModifier.PRIVATE)
    ...
    private fun TypeSpec.Builder.addHelpers() {
        ...
        addFunction(
            FunSpec.builder("err")
                ...
                .returns(KotlinTypes.nothing)
                .addStatement("return error(%P)", "no deserializer registered for _id `\$id`")
                .build()
        )

        val deserializer = JacksonTypes.Databind.jsonDeserializer.parameterizedBy(KotlinTypes.T)
        addFunction(
            FunSpec.builder("map")
                .addModifiers(KModifier.PRIVATE)
                .addTypeVariable(TypeVariableName("T", listOf(symbols.base)))
                .addParameter(ParameterSpec("mapping", mappingInterfaceName.parameterizedBy(KotlinTypes.T)))
