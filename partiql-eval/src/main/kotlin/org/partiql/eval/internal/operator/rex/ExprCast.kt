...
internal class ExprCast(val arg: Operator.Expr, val cast: Ref.Cast) : Operator.Expr {
    ...
    override fun eval(env: Environment): PartiQLValue {
        ...
        try {
            return when (arg.type) {
                ...
                is NullType -> castFromNull(arg as NullValue, cast.target)
    ...
    @OptIn(PartiQLValueExperimental::class)
    private fun castFromNull(value: NullValue, t: PartiQLType): PartiQLValue {
        return when (t) {
            DynamicType -> value
            BoolType -> boolValue(null)
            is CharType -> charValue(null) // TODO: Length
            is CharVarType -> stringValue(null) // TODO: Length
            is CharVarUnboundedType -> stringValue(null)
            is BitType -> TODO()
            is BitVaryingType -> TODO()
            is BlobType -> binaryValue(null)
            is ByteType -> byteValue(null) // TODO: Do we need?
            is ClobType -> clobValue(null) // TODO: Length
            is ClobUnboundedType -> clobValue(null)
            is DateType -> dateValue(null)
            is TimeType -> timeValue(null)
            is TimestampType -> timestampValue(null)
            is IntervalType -> TODO("Not yet supported")
            is BagType -> bagValue<PartiQLValue>(null)
            is ArrayType -> listValue<PartiQLValue>(null)
            is TupleType -> structValue<PartiQLValue>(null)
            is NullType -> value
            is MissingType -> missingValue() // TODO: Os this allowed
            is Int8Type -> int8Value(null)
            is Int16Type -> int16Value(null)
            is Int32Type -> int32Value(null)
            is Int64Type -> int64Value(null)
            is TypeIntBig -> intValue(null)
            is NumericType -> decimalValue(null, t.precision, t.scale)
            is TypeNumericUnbounded -> decimalValue(null)
            is TypeReal -> float32Value(null)
            is TypeDoublePrecision -> float64Value(null)
            is TimeWithTimeZoneType -> TODO()
            is TimestampWithTimeZoneType -> TODO()
            is PartiQLType.Runtime.Custom -> TODO()
        }
    }
