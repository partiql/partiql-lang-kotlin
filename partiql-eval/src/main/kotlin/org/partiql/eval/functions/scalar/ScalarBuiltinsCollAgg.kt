...
internal val SCALAR_BUILTINS_COLL_AGG = listOf(
    PartiQLFunctionCollMax,
    PartiQLFunctionCollMin,
    PartiQLFunctionCollAvg,
    PartiQLFunctionCollSum,
    PartiQLFunctionCollCount,
    PartiQLFunctionCollEvery,
    PartiQLFunctionCollAny,
    PartiQLFunctionCollSome,
)
...
internal sealed class CollectionAggregationFunction(
    val name: String,
    val accumulator: ((PartiQLValue) -> Boolean) -> Accumulator,
) : PartiQLFunction {
    ...
    private fun PartiQLValue.asQuantifierFilter() = when (stringValue().lowercase().trim()) {
        "all" -> { _: PartiQLValue -> true }
        "distinct" -> createUniquePartiQLValueFilter()
        else -> throw IllegalArgumentException("Unrecognized set quantifier: $this")
    }
}

internal object PartiQLFunctionCollMax : CollectionAggregationFunction(
    name = "max",
    accumulator = ::AccumulatorMax,
)

internal object PartiQLFunctionCollMin : CollectionAggregationFunction(
    name = "min",
    accumulator = ::AccumulatorMin
)

internal object PartiQLFunctionCollAvg : CollectionAggregationFunction(
    name = "avg",
    accumulator = ::AccumulatorAvg
)

internal object PartiQLFunctionCollSum : CollectionAggregationFunction(
    name = "sum",
    accumulator = ::AccumulatorSum
)

internal object PartiQLFunctionCollCount : CollectionAggregationFunction(
    name = "count",
    accumulator = ::AccumulatorCount
)

internal object PartiQLFunctionCollEvery : CollectionAggregationFunction(
    name = "every",
    accumulator = ::AccumulatorEvery
)

internal object PartiQLFunctionCollAny : CollectionAggregationFunction(
    name = "any",
    accumulator = ::AccumulatorAnySome
)

internal object PartiQLFunctionCollSome : CollectionAggregationFunction(
    name = "some",
    accumulator = ::AccumulatorAnySome
)
