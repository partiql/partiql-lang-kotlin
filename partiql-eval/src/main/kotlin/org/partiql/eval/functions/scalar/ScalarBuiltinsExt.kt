...
internal val SCALAR_BUILTINS_EXT = listOf(
    PartiQLFunctionExists,
    PartiQLFunctionUtcNow,
    PartiQLFunctionFilterDistinct,
    PartiQLFunctionDateAdd,
    PartiQLFunctionDateDiff,
    PartiQLFunctionMakeDate,
    PartiQLFunctionMakeTime_1,
    PartiQLFunctionMakeTime_2,
    PartiQLFunctionToTimestamp_1,
    PartiQLFunctionToTimestamp_2,
    PartiQLFunctionSize,
    PartiQLFunctionFromUnix,
    PartiQLFunctionUnixTimestamp_1,
    PartiQLFunctionUnixTimestamp_2,
    PartiQLFunctionToString,
    PartiQLFunctionTextReplace,
)
 ...
 */
internal object PartiQLFunctionExists : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return PartiQLValue.newBoolean(result)
    }
}
 ...
 */
internal object PartiQLFunctionUtcNow : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        return PartiQLValue.newTimestamp(session.now)
    }
}
 ...
 */
internal object PartiQLFunctionFilterDistinct : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        // We cannot use a [HashSet] here because [PartiQLValue] does not implement .equals() and .hashCode()
        val encountered = TreeSet(DEFAULT_COMPARATOR)
        return PartiQLValue.newBag(
            sequence {
 ...
 * * second
 */
internal object PartiQLFunctionDateAdd : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return try {
            ...
            PartiQLValue.newTimestamp(result)
 ...
 */
internal object PartiQLFunctionDateDiff : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return PartiQLValue.newInt(result.toLong())
    }
}

/**
 * Creates a DATE PartiQLValue from the date fields year, month and day.
 ...
 */
internal object PartiQLFunctionMakeDate : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        val (year, month, day) = required.map {
            ...
            if (it.type != PartiQLValueType.INT) {
                err(
        ...
        return try {
            PartiQLValue.newDate(year, month, day)
...
/**
 * Creates a TIME PartiQLValue from the time fields hour, minute, second and optional timezone_minutes.
...
internal abstract class PartiQLFunctionMakeTime : PartiQLFunction {
    protected fun makeTime(
        hour: Int,
        minute: Int,
        second: BigDecimal,
        tzMinutes: Int?
...
internal object PartiQLFunctionMakeTime_1 : PartiQLFunctionMakeTime() {
    override val signature = FunctionSignature(
        name = "make_time",
...
internal object PartiQLFunctionMakeTime_2 : PartiQLFunctionMakeTime() {
    override val signature = FunctionSignature(
        name = "make_time",
 ...
 */
internal abstract class PartiQLFunctionToTimestamp : PartiQLFunction

internal object PartiQLFunctionToTimestamp_1 : PartiQLFunctionToTimestamp() {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return PartiQLValue.newTimestamp(ts)
    }
}

internal object PartiQLFunctionToTimestamp_2 : PartiQLFunctionToTimestamp() {
 ...
 */
internal object PartiQLFunctionSize : PartiQLFunction {
...
/**
 * Builtin function to convert the given unix epoch into a PartiQL `TIMESTAMP` [PartiQLValue]. A unix epoch represents
 ...
 * fractional seconds.
 *
 * When given a negative numeric value, this function returns a PartiQL `TIMESTAMP` [PartiQLValue] before the last epoch.
 * When given a non-negative numeric value, this function returns a PartiQL `TIMESTAMP` [PartiQLValue] after the last
 * epoch.
 */
internal object PartiQLFunctionFromUnix : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return PartiQLValue.newTimestamp(timestamp)
    }
}

/**
 * Builtin function to convert the given PartiQL `TIMESTAMP` [PartiQLValue] into a unix epoch, where a unix epoch
 ...
 * '1970-01-01 00:00:00' UTC as a PartiQL `INT` [PartiQLValue]
 *
 ...
 */
internal abstract class PartiQLFunctionUnixTimestamp : PartiQLFunction {
    private val millisPerSecond = BigDecimal(1000)
    protected fun epoch(timestamp: Timestamp): BigDecimal = timestamp.decimalMillis.divide(millisPerSecond)
}

internal object PartiQLFunctionUnixTimestamp_1 : PartiQLFunctionUnixTimestamp() {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        return PartiQLValue.newInt(epoch(session.now).toLong())
    }
}

internal object PartiQLFunctionUnixTimestamp_2 : PartiQLFunctionUnixTimestamp() {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return if (timestamp.decimalSecond.scale() == 0) {
            PartiQLValue.newInt(epochTime.toLong())
        } else {
            PartiQLValue.newDecimal(epochTime)
        }
    }
}
 ...
 */
internal object PartiQLFunctionToString : PartiQLFunction {
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        try {
            return PartiQLValue.newString(formatter.format(temporalAccessor))
 ...
 */
internal object PartiQLFunctionTextReplace : PartiQLFunction {
    override val signature = FunctionSignature(
        name = "text_replace",
    ...
    override fun invoke(session: ConnectorSession, arguments: List<PartiQLValue>): PartiQLValue {
        ...
        return PartiQLValue.newString(string.replace(from, to))
    }
}
