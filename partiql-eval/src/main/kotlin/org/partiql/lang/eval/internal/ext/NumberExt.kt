...
internal fun Number.ionValue(ion: IonSystem): IonValue = when (this) {
...
internal operator fun Decimal.unaryMinus(): Decimal = when {
...
internal operator fun Number.rem(other: Number): Number {
    val (first, second) = coerceNumbers(this, other)
    return when (first) {
 ...
 */
internal fun BigDecimal.squareRoot(mathContext: MathContext = MATH_CONTEXT): BigDecimal {
    if (this.signum() < 0) {
        throw ArithmeticException("Cannot take root of a negative number")
    }
 ...
 */
internal fun BigDecimal.exp(mathContext: MathContext = MATH_CONTEXT): BigDecimal {
    val operationMC = MathContext(
 ...
 */
internal fun BigDecimal.ln(mathContext: MathContext = MATH_CONTEXT): BigDecimal {
    if (this.signum() <= 0) {
...
internal fun BigInteger.transformIntType(): Number = when (this) {
