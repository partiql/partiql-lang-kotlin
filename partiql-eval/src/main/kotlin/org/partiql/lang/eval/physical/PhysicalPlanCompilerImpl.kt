...
import org.partiql.lang.eval.internal.errorIf
import org.partiql.lang.eval.internal.ext.checkThreadInterrupted
...
internal class PhysicalPlanCompilerImpl(
    ...
    private val thunkFactory =
        evaluatorOptions.typingMode.createThunkFactory<EvaluatorState>(evaluatorOptions.thunkOptions)
    ...
    private fun compileIn(expr: PartiqlPhysical.Expr.InCollection, metas: MetaContainer): PhysicalPlanThunk {
        ...
        fun isOptimizedCase(values: List<PartiqlPhysical.Expr>): Boolean =
            values.all { it is PartiqlPhysical.Expr.Lit && !it.value.isNull }
    ...
    private fun compileCall(expr: PartiqlPhysical.Expr.Call, metas: MetaContainer): PhysicalPlanThunk {
        ...
        return thunkFactory.thunkEnv(metas) { env ->
            ...
            } catch (e: org.partiql.lang.eval.internal.ArityMismatchException) {
    ...
    private fun compileCastHelper(
        value: PartiqlPhysical.Expr,
        asType: PartiqlPhysical.Type,
        metas: MetaContainer,
    ): PhysicalPlanThunk {
    ...
    private fun compileCanLosslessCast(
        expr: PartiqlPhysical.Expr.CanLosslessCast,
        metas: MetaContainer,
    ): PhysicalPlanThunk {
        val typedOpParameter = expr.asType.toTypedOpParameter(customTypedOpParameters)
        if (typedOpParameter.staticType is AnyType) {
    ...
    private fun compileSeq(
        seqType: ExprValueType,
        itemExprs: List<PartiqlPhysical.Expr>,
        metas: MetaContainer,
    ): PhysicalPlanThunk {
