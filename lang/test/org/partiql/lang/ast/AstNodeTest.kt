...
class AstNodeTest {
    ...
    fun pathChildren() {
        val root = literal("1")
        val component1 = PathComponentExpr(literal("2"), CaseSensitivity.INSENSITIVE, emptyMeta)
        val component2 = PathComponentExpr(literal("3"), CaseSensitivity.INSENSITIVE, emptyMeta)
    ...
    @Test
    fun selectChildren() {
        val projection = SelectProjectionValue(literal("1"), emptyMeta)
    ...
    fun pathComponentExprChildren() {
        val child = literal("2")
        assertEquals(listOf(child), PathComponentExpr(child, CaseSensitivity.INSENSITIVE, emptyMeta).children)
    }
    ...
    fun selectProjectionListChildren() {
        ...
        assertEquals(listOf(child1, child2), SelectProjectionList(listOf(child1, child2), emptyMeta).children)
    }
    ...
    fun selectProjectionValueChildren() {
        ...
        assertEquals(listOf(child), SelectProjectionValue(child, emptyMeta).children)
    }
    ...
    fun selectProjectionPivotChildren() {
        ...
        assertEquals(listOf(child1, child2), SelectProjectionPivot(child1, child2, emptyMeta).children)
    }
