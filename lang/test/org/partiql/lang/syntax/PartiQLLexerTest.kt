...
// TODO: Delete class
class PartiQLLexerTest : TestBase() {
    private val lexer = PartiQLLexer(ion)
    ...
    fun whitespaceAndIdentifiers() = assertTokens(
        ...
        token(TokenType.IDENTIFIER, "\$cd\$", 2, 10, 4),
        token(TokenType.IDENTIFIER, "de1", 3, 3, 3)
    )
    ...
    fun whitespaceAndQuotedIdentifiers() = assertTokens(
        ...
        token(TokenType.QUOTED_IDENTIFIER, "\$cd\$", 2, 12, 6),
        token(TokenType.QUOTED_IDENTIFIER, "de1", 3, 3, 5)
    )
    ...
    fun multiLexemeOperators() = assertTokens(
        ...
        token(TokenType.OPERATOR, "union_all", "UNION ALL", 1, 1, 9),
        token(TokenType.OPERATOR, "is_not", "IS NOT", 1, 11, 6),
    ...
    fun multiLexemeKeywords() = assertTokens(
        ...
        token(TokenType.KEYWORD, "character_varying", "CHARACTER VARYING", 1, 1, 17),
        token(TokenType.KEYWORD, "double_precision", "DoUblE PrEcision", 1, 19, 16),
    ...
    fun joinKeywords() = assertTokens(
        ...
        token(TokenType.KEYWORD, "cross_join", "cRoSs Join", 1, 1, 10),
        token(TokenType.KEYWORD, "left_join", "left join", 1, 12, 9),
        token(TokenType.KEYWORD, "left", "left", 1, 22, 4),
        token(TokenType.KEYWORD, "left", "left", 1, 27, 4),
        token(TokenType.KEYWORD, "inner_join", "Inner joiN", 1, 32, 10),
        token(TokenType.KEYWORD, "right_join", "RIGHT JOIN", 1, 43, 10),
        token(TokenType.KEYWORD, "right_join", "RIGHT OUTER JOIN", 1, 54, 16),
        token(TokenType.KEYWORD, "left_join", "LEFT OUTER JOIN", 1, 71, 15),
        token(TokenType.KEYWORD, "outer_join", "FULL join", 2, 1, 9),
        token(TokenType.KEYWORD, "outer_join", "OUTER JOIN", 2, 11, 10),
        token(TokenType.KEYWORD, "outer_join", "FULL OUTER JOIN", 2, 22, 15),
        token(TokenType.KEYWORD, "cross", "CROSS", 3, 1, 5),
        token(TokenType.KEYWORD, "cross_join", "CROSS JOIN", 3, 7, 10),
    ...
    fun doublePrecisionType() = assertTokens(
        "DOUBLE PRECISION",
        token(TokenType.KEYWORD, "double_precision", "DOUBLE PRECISION", 1, 1, 16)
    )
