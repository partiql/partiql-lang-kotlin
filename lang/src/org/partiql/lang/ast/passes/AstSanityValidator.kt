 ...
 */
object AstSanityValidator {
    fun validate(astNode: AstNode) {
        astNode.forEach { node -> 
            when(node) {
                is Literal -> {
                    val (ionValue, metas: MetaContainer) = node
                ...
                is SelectProjectionList -> {
                    val asterisk = node.items.filterIsInstance<SelectListItemStar>().firstOrNull()

                    // If the select list contains more than one item and one of them is a `*`
                    if(asterisk != null && node.items.size > 1) {
                ...
                is DataType -> {
                    val (sqlDataType, args, _: MetaContainer) = node
                    if(!sqlDataType.arityRange.contains(args.size)) {
                        checkArity(sqlDataType.name, sqlDataType.arityRange, args.size, node.metas)
                    }
                }
