...
internal open class ParserBenchmark {
    ...
    fun parseSuccess(state: MyState, blackhole: Blackhole) {
        val expr = state.parser.parseAstStatement(state.queries[state.queryIndex])
    ...
    @Measurement(iterations = MEASUREMENT_ITERATION_VALUE, time = MEASUREMENT_TIME_VALUE)
    @Warmup(iterations = WARMUP_ITERATION_VALUE, time = WARMUP_TIME_VALUE)
    fun parseFail50Percent(state: MyState, blackhole: Blackhole) {
        try {
            val expr = state.parser.parseAstStatement(state.queriesFailAt50Percent[state.queryIndex])
            blackhole.consume(expr)
    ...
    open class MyState {
        ...
        val queriesFailAt50Percent = queries.map { query ->
