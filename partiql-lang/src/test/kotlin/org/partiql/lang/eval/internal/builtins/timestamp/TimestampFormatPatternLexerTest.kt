...
import org.partiql.lang.eval.internal.timestamp.TimestampFormatPatternLexer
import org.partiql.lang.eval.internal.timestamp.Token
import org.partiql.lang.eval.internal.timestamp.TokenType
...
class TimestampFormatPatternLexerTest {
    ...
    private fun assertTokens(s: String, vararg tokens: Token) = assertEquals(
        tokens.toList(),
    ...
    @Test
    fun allPatternCharacters() = assertTokens(
        "yMdahHmsSXx",
        pattern("y"),
        pattern("M"),
        pattern("d"),
    ...
    @Test
    fun onlyText() = assertTokens(
        "'some quoted text'-----'more quoted'.''''",
    ...
    @Test
    fun withWhitespace() = assertTokens(
        "y y y y",
        pattern("y"),
        text(" "),
        pattern("y"),
    ...
    @Test
    fun withNonEscapedTextAndPattern() = assertTokens(
        "y/m-d,h:y.s",
        pattern("y"),
        text("/"),
        pattern("m"),
    ...
    @Test
    fun withNonEscapedTextWhitespaceAndPattern() = assertTokens(
        "yyyy-mm-dd HH:hh",
        pattern("yyyy"),
        text("-"),
        pattern("mm"),
    ...
    @Test
    fun ionTimestampDefaultPattern() = assertTokens(
