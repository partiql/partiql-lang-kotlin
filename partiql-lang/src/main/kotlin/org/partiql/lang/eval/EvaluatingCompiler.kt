...
internal class EvaluatingCompiler(
    ...
    private class CompiledGroupByItem(val alias: ExprValue, val uniqueId: Ident?, val thunk: ThunkEnv)

    /**
     * Represents a memoized binding [BindingName] and an [ExprValue] of the same name.
     * Used during evaluation og `GROUP BY`.
     */
    private data class FromSourceBindingNamePair(val bindingName: Ident, val nameExprValue: ExprValue)
    ...
    private fun compileVr(expr: PartiqlAst.Expr.Vr, metas: MetaContainer): ThunkEnv {
        ...
        return when (uniqueNameMeta) {
            null -> {
                val bindingName = expr.id.toIdent()
                            ...
                            when (val value = env.current[bindingName]) {
                                null -> {
                                    if (fromSourceNames.any { bindingName.isEquivalentTo(it) }) {
                                        err(
                                            "Variable not in GROUP BY or aggregation function: ${bindingName.toDisplayString()}",
                                        ...
                                        err(
                                            "No such binding: ${bindingName.toDisplayString()}.$hint",
                                            errorCode,
                                            errorContextFrom(metas).also {
                                                it[Property.BINDING_NAME] = bindingName.toDisplayString()
                                            },
                ...
                // wVG val bindingName = BindingName(uniqueNameMeta.uniqueName, BindingCase.SENSITIVE)
                val bindingName = Ident.createAsIs(uniqueNameMeta.uniqueName)
                ...
                thunkFactory.thunkEnv(metas) { env ->
                    ...
                    env.current[bindingName] ?: err(
                        "Uniquely named binding \"${bindingName.toDisplayString()}\" does not exist for some reason",
                            ...
                            groupByItems.isEmpty() -> { // There are aggregates but no group by items
                                ...
                                val fromSourceBindingNames = fromSourceThunks.map {
                                    FromSourceBindingNamePair(
                                        // wVG BindingName(it.alias.asName.underlyingString(), BindingCase.SENSITIVE),
                                        it.alias.asName,
    ...
    private fun compileGroupByExpressions(groupByItems: List<PartiqlAst.GroupKey>): List<CompiledGroupByItem> =
        groupByItems.map {
            ...
            val uniqueId = uniqueName?.let { Ident.createAsIs(it) }

            CompiledGroupByItem(alias.string().exprValue(), uniqueId, compileAstExpr(it.expr))
        }
    ...
    private fun compileGroupKeyThunk(compiledGroupByItems: List<CompiledGroupByItem>, selectMetas: MetaContainer) =
        thunkFactory.thunkEnv(selectMetas) { env ->
            val uniqueNames = HashMap<Ident, ExprValue>(compiledGroupByItems.size, 1f)
                                ...
                                thunkFactory.thunkEnvValue(componentMetas) { env, componentValue ->
                                    ...
                                    when {
                                        ...
                                        indexValue.type.isText -> {
                                            val lookupName =
                                                indexValue.stringValue().toIdent(idKind)
