...
internal class PartiQLVisitor(
    ...
    override fun visitDropTable(ctx: PartiQLParser.DropTableContext) = PartiqlAst.build {
        ...
        val prefix = allSteps.dropLast(1)
        val prefixNode = if (prefix.isNotEmpty()) {
            PartiqlAst.build {
                tableNamePrefix(prefix)
            }
        } else null
        dropTable(prefixNode, name, ctx.DROP().getSourceMetaContainer())
    }
    ...
    override fun visitCreateTable(ctx: PartiQLParser.CreateTableContext) = PartiqlAst.build {
        ...
        val prefixNode = if (prefix.isNotEmpty()) {
            PartiqlAst.build {
                tableNamePrefix(
                    prefix.map { it.toIdentifier() }
                )
            }
        } else null
        val def = ctx.tableDef()?.let { visitTableDef(it) }
        createTable_(prefixNode, name, def, ctx.CREATE().getSourceMetaContainer())
    }
