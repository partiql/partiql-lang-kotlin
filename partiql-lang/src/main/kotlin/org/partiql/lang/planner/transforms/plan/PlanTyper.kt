...
internal object PlanTyper : PlanRewriter<PlanTyper.Context>() {
    ...
    internal class Context(
        ...
        internal val allFunctions: Map<String, FunctionSignature>,
        ...
        internal val metadata = plannerCtx.metadata
    }
    ...
    private fun findBind(path: BindingPath, qualifier: Rex.Id.Qualifier, ctx: Context): ReferenceResolver.ResolvedDescriptor {
        ...
        return when (scopingOrder) {
            ScopingOrder.GLOBALS_THEN_LEXICAL -> ReferenceResolver.resolveGlobalBind(path, ctx)
                ...
                ?: handleUnresolvedDescriptor(path.steps[0], ctx) {
    ...
    private fun handleExpressionAlwaysReturnsNullOrMissingError(ctx: Context) {
        ctx.problemHandler.handleProblem(
            Problem(
                sourceLocation = unknownSourceLocationMeta(),
    ...
    private fun unknownSourceLocationMeta() = SourceLocationMeta(-1, -1)
}
