 ...
 */
internal fun PartiqlPhysical.Type.toTypedOpParameter(customTypedOpParameters: Map<String, TypedOpParameter>): TypedOpParameter =
    when (this) {
        ...
        is PartiqlPhysical.Type.FloatType, is PartiqlPhysical.Type.RealType, is PartiqlPhysical.Type.DoublePrecisionType -> TypedOpParameter(
            StaticType.FLOAT
        )
        ...
        is PartiqlPhysical.Type.CharacterVaryingType -> when (val length = this.length) {
        ...
        is PartiqlPhysical.Type.CustomType ->
            customTypedOpParameters.mapKeys { (k, _) ->
                Ident.normalizeRegular(k)
            }[Ident.normalizeRegular(this.name.string())] ?: error("Could not find parameter for $this")
