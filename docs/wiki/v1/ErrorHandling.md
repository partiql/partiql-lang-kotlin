...
class AbortWhenAlwaysMissing extends ErrorListener {
    ...
    void warning(@NotNull Error error) throws ErrorListenerException {
        if (error.getCode() == ErrorCode.ALWAYS_MISSING) {
...
`ErrorCode.INTERNAL_ERROR`. This will lead to a duplication of errors (which can be a bad experience for your
customers).
...
the [ErrorCode Javadocs](https://github.com/partiql/partiql-lang-kotlin/wiki/TODO).
...
each `ErrorCode`'s and `Property`'s Javadocs before using them in your application.
