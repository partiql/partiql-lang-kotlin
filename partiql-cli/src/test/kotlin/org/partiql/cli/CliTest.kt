...
class CliTest {
    ...
    fun runQueryOnSingleValue() {
        ...
        val ionInputResult = makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION)
        val partiqlInputResult = makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.PARTIQL)
    ...
    fun specifyingWrapIonWithPartiQLInput() {
        ...
        assertThrows<IllegalArgumentException> {
            makeCliAndGetResult(query, input, wrapIon = true, inputFormat = MainCommand.InputFormat.PARTIQL)
        }
    }
    ...
    fun withBinding() {
        ...
        val partiqlInputResult = makeCliAndGetResult(query, input, bindings = bindings, inputFormat = MainCommand.InputFormat.PARTIQL)
    ...
    fun withPartiQLOutput() {
        ...
        val wrappedInputResult = makeCliAndGetResult(query, wrappedInput, wrapIon = true, outputFormat = MainCommand.OutputFormat.PARTIQL)
        val ionInputResult = makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION, outputFormat = MainCommand.OutputFormat.PARTIQL)
    ...
    fun withPartiQLPrettyOutput() {
        ...
        val actual = makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION, outputFormat = MainCommand.OutputFormat.PARTIQL_PRETTY)
    ...
    fun withIonTextOutput() {
        ...
        val actual = makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION, outputFormat = MainCommand.OutputFormat.ION_TEXT)
    ...
    fun withIonTextOutputToFile() {
        ...
        val input = "[{'a': 1}, {'b': 1}]"
        val expected = "$BAG_ANNOTATION::[{a:1}\n,{b:1}\n]"
        makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION, outputFormat = MainCommand.OutputFormat.ION_TEXT, output = FileOutputStream(testFile))
        val ionInputResult = testFile!!.bufferedReader().use { it.readText() }
        assertAsIon(expected, ionInputResult)

        makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.PARTIQL, outputFormat = MainCommand.OutputFormat.ION_TEXT, output = FileOutputStream(testFile))
    ...
    fun runWithProjectionIterationFilterMissingFailure() {
        ...
        assertThrows<EvaluationException> {
            makeCliAndGetResult(query, input, pipeline = pipeline, inputFormat = MainCommand.InputFormat.PARTIQL)
        }
    }
    ...
    fun runWithProjectionIterationFilterMissingSuccess() {
        ...
        val input = "<<{'a': null, 'b': missing, 'c': 1}>>"
        val query = "SELECT * FROM input_data"
        val actual = makeCliAndGetResult(query, input, pipeline = pipeline, inputFormat = MainCommand.InputFormat.PARTIQL)
    ...
    fun partiqlInputFailure() {
        ...
        assertThrows<IonException> {
            makeCliAndGetResult(query, input, inputFormat = MainCommand.InputFormat.ION)
        }
    }
}
