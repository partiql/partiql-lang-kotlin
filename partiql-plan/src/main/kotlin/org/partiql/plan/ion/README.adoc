...
----
(
  partiql::(<MAJOR> <MINOR>)     // MAJOR.MINOR version int pair

  (include ...)                  // include prelude

  (types ...)                    // type definitions

  (constants ...)                // plan constant values

  (define ...)                   // plan node definitions
...
| expression | The root expression node of a query.
...
[source,ion]
----
(create_view name::string (columns string*) | query::rex)
----
...
|<<bag-rex-node,bag>>
| Bag constructor expression

|<<array-rex-node,array>>
| Array constructor expression

|<<pivot-rex-node,pivot>>
| Pivot expression to produce a tuple from a binding collection.

|<<query-rex-node,select>>
...
NOTE: I find the `<int>` type references difficult to describe by hand, but I suppose it's for ease of machine readability.
...
[source,ion]
----
(global name::string type::type (case symbol))
----
...
| type::type | PartiQL Type

| (case symbol) |
Optional binding name case. One of `SENSITIVE` or `INSENSITIVE`
...
[source,ion]
----
(global "MY_TABLE" (bag tuple))

(global "My_Table" (case SENSITIVE))
----
...
| type::type | The variable's PartiQL Type.

2+a|

NOTE: Do we want ordinal to be `id` (or something else `name` or `binding`) rather than `ordinal` as to not imply orderedness of the variable binding environment?
...
A tuple constructor differs from a tuple literal because expressions may be used to _construct_ the tuple value. Tuple key expressions must evaluate to a value with character string type.

.Form
[source,ion]
----
(tuple | (key::rex val::rex)*)
----
...
| (key::rex val::rex)* | Zero or more key-value expression pairs as the tail
...
The pivot node is an expression which produces a single tuple from a query. Each key-value pair in the output tuple is produced by evaluating the given key and value expressions for each binding tuple produced by the relational operator. For more details, see <<scalar_subquery_pivot,PIVOT Clause>>.

.Form
[source,ion]
----
(pivot key::rex val::rex | op::operator)
----
...
| op::operator | Relational operator node which represents the SELECT-FROM-WHERE query.

|===

Recall the example environment,

.Example
[source,partiql]
----
PIVOT val at name FROM MY_TABLE
...
[source,ion]
----
(pivot (var 0 string(30)) (var 1 int8)
  (scan (global "MY_TABLE"))
)
----
...
I have chosen the name `select` to represent a SELECT-FROM-WHERE expression (query expression) in the plan node. The symbol `query` already represents a query statement. Considering a `select` node takes a value constructor as well as projects from the relation domain (domain of binding tuples) to domain of PartiQL values, I considered naming this a `value` node. Which would somewhat align with `SELECT VALUE` .. as it's the general form for PartiQL's relation-to-value projection.

.Form
[source,ion]
----
(select constructor::expression | op::operator)
----
...
| (constructor expression) | Optional constructor expression

| operator | Relational operator node
...
-- Example 1
SELECT VALUE  { name: val } FROM "MY_TABLE" WHERE val > 2

-- << { 'three': 3 }, { 'four': 4 } >>

-- Example 2
SELECT val as name FROM "MY_TABLE" WHERE val > 2
...
(select
  (constructor
    (tuple
      ((var 0 string(30)) (var 1 int8))
    )
  )
  (filter
    (call gt (var 1 int8) (lit int 2))
    (scan (global "MY_TABLE"))
  )
)
...
| (index bool) | If true, the `(var 0)` binding will be the collection index. This corresponds to the `AT` of a <<from-clause,PartiQL FROM Clause>>. I'm consider having the syntax `(index)` and `(index true)` being equivalent.

| expression | Expression may be any PartiQL expression. This enables

|===

Somewhere I feel we need a mapping from values to binding tuples — ie a way to specify how named (string-value) pairs map to ordered values.
...
// Somewhere we need to map named pairs to integers
// For example, we subsitute "x"->0 "y"->1
...
[source,ion]
----
(unpivot (at expression) (by expression) | expression)
----
...
| (at expression) | Optional expression for UNPIVOT AT

| (by expression) | Optional expression for UNPIVOT BY

| expression | Value expression to apply the unpivot operator to
