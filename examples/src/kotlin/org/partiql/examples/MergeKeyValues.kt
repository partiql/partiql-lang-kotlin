...
class MergeKeyValues : MergeKeysBaseExprFunction() {
    ...
    override fun callWithRequired(session: EvaluationSession, required: List<ExprValue>): ExprValue {
        ...
        val keys = result.keys.map { stringExprValue(it) }
        val values = result.values.map { listExprValue(it).flatten() }

        val listOfStructs = keys.zip(values)
            .map {
                structExprValue(
                    listOf(listExprValue(it.second).namedValue(it.first)).asSequence(),
                    StructOrdering.UNORDERED
                )
            }
        return listExprValue(listOfStructs)
    }
}
