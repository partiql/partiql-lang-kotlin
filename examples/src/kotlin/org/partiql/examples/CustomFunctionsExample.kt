...
class FibListExprFunc : ExprFunction {
    ...
    override fun callWithRequired(session: EvaluationSession, required: List<ExprValue>): ExprValue {
        ...
        val listElements = (0L..n).map { i ->
            ...
            val fieldValue = intExprValue(calcFib(i)).namedValue(stringExprValue("n"))
            structExprValue(sequenceOf(fieldValue), StructOrdering.UNORDERED)
        }.asSequence()

        return listExprValue(listElements)
    }
}
