...
import org.partiql.planner.impl.sourceLocationMeta
...
public class AggregationVisitorTransform(
    ...
    internal companion object {
        internal const val GROUP_PREFIX: String = "\$__partiql__group_by_"
        internal const val GROUP_DELIMITER: String = "_"
    ...
    override fun transformSortSpec_expr(node: PartiqlAst.SortSpec): PartiqlAst.Expr = itemTransform.transformSortSpec_expr(node)
    ...
    private class GroupKeyReferenceTransformer(
        ...
        override fun transformExprId(node: PartiqlAst.Expr.Id): PartiqlAst.Expr = when (this.isWithinCallAgg) {
        ...
        private fun getReplacementExpression(node: PartiqlAst.Expr): PartiqlAst.Expr? {
            ...
            while (ctxStackIter.hasPrevious()) {
                ...
                ctx.groupKeys.firstOrNull { key ->
                    ...
                    return PartiqlAst.build {
                        id(key.uniqueAlias, caseSensitive(), unqualified(), emptyMetaContainer())
                    }
                }
            }
            return null
         ...
         */
        private fun getReplacementForIdInAggregationFunction(node: PartiqlAst.Expr.Id): PartiqlAst.Expr {
        ...
        private fun getReplacementInNormalContext(node: PartiqlAst.Expr.Id, ctx: VisitorContext): PartiqlAst.Expr.Id? {
            ...
            return when {
                ...
                bindingName.isEquivalentTo(ctx.groupAsAlias) -> PartiqlAst.build { id(node.name.text, caseSensitive(), unqualified()) }
                else -> null
            }
        }
    ...
    public data class GroupKeyInformation(
