...
import org.partiql.planner.impl.errAstNotNormalized
import org.partiql.planner.impl.errorContextFrom
import org.partiql.planner.impl.handleUnimplementedFeature
import org.partiql.planner.impl.metaContainerOf
...
public class AstToLogicalVisitorTransform(
    public val problemHandler: ProblemHandler
) : PartiqlAstToPartiqlLogicalVisitorTransform() {

    internal companion object {
        internal const val EXCLUDED: String = "EXCLUDED"
        internal const val COLL_AGG_FUNCTION_PREFIX = "coll_"
        internal const val CURRENT_USER_KEY = "CURRENT_USER"
        internal const val CURRENT_USER_FUNCTION_NAME: String = "\$__current_user"
    }
    ...
    override fun transformExprSessionAttribute(node: PartiqlAst.Expr.SessionAttribute): PartiqlLogical.Expr.Call = PartiqlLogical.build {
        val functionName = when (node.value.text.uppercase()) {
            CURRENT_USER_KEY -> CURRENT_USER_FUNCTION_NAME
            else -> err(
    ...
    override fun transformExprCallAgg(node: PartiqlAst.Expr.CallAgg): PartiqlLogical.Expr = PartiqlLogical.build {
        call(
            "$COLL_AGG_FUNCTION_PREFIX${node.funcName.text}",
            listOf(
...
private class FromSourceToBexpr @OptIn(ExperimentalPartiQLPlanner::class) constructor(
