...
import org.partiql.ast.v1.SetQuantifier as AstSetQuantifier

/**
 * Converts an AST expression node to a Plan Rex node; ignoring any typing.
 */
internal object V1RexConverter {
    ...
    private object ToRex : AstVisitor<Rex, Env> {
        ...
        override fun visitExprVarRef(node: ExprVarRef, context: Env): Rex {
            val type = (ANY)
            val identifier = V1AstToPlan.convert(node.identifierChain)
        ...
        override fun visitExprPath(node: ExprPath, context: Env): Rex {
            ...
            val (newRoot, newSteps) = when (val op = root.op) {
                is Rex.Op.Var.Unresolved -> {
                    ...
                    val newSteps = mutableListOf<PathStep>()
                    var curStep = node.next
                    while (curStep != null) {
                        if (curStep !is PathStep.Field) {
                            break
                        }
                        parts.add(V1AstToPlan.part(curStep.field))
                        newSteps.add(curStep)
                        i += 1
                    ...
                    val allSteps = mutableListOf<PathStep>()
                    var curStep = node.next
                    while (curStep != null) {
                        allSteps.add(curStep)
                        curStep = curStep.next
                    }
                    root to allSteps
                }
        ...
        override fun visitExprCall(node: ExprCall, context: Env): Rex {
            val type = (ANY)
            // Fn
            val id = V1AstToPlan.convert(node.function)
            if (id.hasQualifier()) {
        ...
        private fun visitExprCallExists(node: ExprCall, context: Env): Rex {
            ...
            val op = rexOpCallUnresolved(V1AstToPlan.convert(node.function), listOf(arg))
            return rex(type, op)
        }
        ...
        override fun visitExprIsType(node: ExprIsType, ctx: Env): Rex {
            ...
            var call = when (targetType.code()) {
                ...
                DataType.INT -> call("is_int", arg0) // TODO ALAN figure out if INT should map to INT4
                ...
                // TODO ALAN other types that are in functions but aren't in AST yet
//                DataType.BYTE_STRING -> call("is_byteString", arg0)
//                DataType.ANY -> call("is_any", arg0)
                else -> TODO() // TODO ALAN
            }
        ...
        override fun visitExprOverlay(node: ExprOverlay, ctx: Env): Rex {
            ...
            val sp = visitExprCoerce(node.placing, ctx)
            val rs = visitExprCoerce(node.from, ctx)
        ...
        private fun visitType(type: DataType): CompilerType {
            return when (type.code()) {
                ...
                DataType.INT4, DataType.INTEGER4, DataType.INTEGER -> PType.integer()
                DataType.INT -> PType.numeric() // TODO ALAN figure out if INT should map to INT4
                ...
                // TODO ALAN other types that are in previous RexConverter but not in AST yet
//                DataType.ByteString -> error("BINARY is not supported yet.")
//                DataType.Any -> PType.dynamic()
                else -> TODO() // TODO ALAN
            }.toCType()
        }
        ...
        // TODO ALAN add add the same validation here or in parser. Ensure the DATE_ADD/DATE_DIFF fn names map to the
        //  same header functions.
...
//        }
        ...
        override fun visitExprQuerySet(node: ExprQuerySet, context: Env): Rex = V1RelConverter.apply(node, context)
