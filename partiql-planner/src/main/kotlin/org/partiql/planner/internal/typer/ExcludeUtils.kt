 ...
 */
internal fun StaticType.checkExclude(steps: List<Rel.Op.Exclude.Step>): Boolean {
    return when (this) {
 ...
 */
internal fun StructType.checkExclude(steps: List<Rel.Op.Exclude.Step>): Boolean {
    // Ignore open structs
        ...
        if (steps.size == 1) {
            when (step) {
                is Rel.Op.Exclude.Step.StructField -> {
                    step.symbol.isEquivalentTo(field.key)
                }
                is Rel.Op.Exclude.Step.StructWildcard -> true
                else -> false
            }
        } else {
            when (step) {
 ...
 */
internal fun CollectionType.checkExclude(steps: List<Rel.Op.Exclude.Step>): Boolean {
    val step = steps.first()
    return if (steps.size == 1) {
        when (step) {
            is Rel.Op.Exclude.Step.CollIndex, is Rel.Op.Exclude.Step.CollWildcard -> true
            else -> false
        }
    } else {
        when (step) {
...
internal fun Rel.Op.Exclude.Item.toProblemString(): String {
    val root = when (root) {
