...
internal object FnResolver {
     ...
     * @return
     */
    fun resolve(variants: List<Routine.Scalar>, args: List<CompilerType>): FnMatch? {
     ...
     * @return
     */
    private fun match(candidates: List<Routine.Scalar>, args: List<CompilerType>): List<MatchResult> {
     ...
     */
    private fun Routine.Scalar.matchesExactly(args: List<CompilerType>): Boolean {
        val parameters = getParameters()
        for (i in args.indices) {
            val a = args[i]
            val p = parameters[i]
            if (a.kind != p.type) return false
        }
        return true
    }
     ...
     * @return
     */
    private fun Routine.Scalar.match(args: List<CompilerType>): MatchResult? {
