...
internal object PlanTransform : PlanBaseVisitor<PlanNode, ProblemCallback>() {
    ...
    override fun visitFn(node: Fn, ctx: ProblemCallback) = super.visitFn(node, ctx) as org.partiql.plan.Fn

    override fun visitFnResolved(node: Fn.Resolved, ctx: ProblemCallback) = org.partiql.plan.fnStatic(node.signature)

    override fun visitFnUnresolved(node: Fn.Unresolved, ctx: ProblemCallback): PlanNode {
        error("Unresolved function: $node")
    }
    ...
    override fun visitAggUnresolved(node: Agg.Unresolved, ctx: ProblemCallback): PlanNode {
        error("Unresolved aggregation: $node")
    }
    ...
    override fun visitIdentifierQualified(node: Identifier.Qualified, ctx: ProblemCallback) =
        org.partiql.plan.Identifier.Qualified(root = visitIdentifierSymbol(node.root, ctx),
    ...
    override fun visitRexOpVar(node: Rex.Op.Var, ctx: ProblemCallback) =
        super.visitRexOpVar(node, ctx) as org.partiql.plan.Rex.Op.Var

    override fun visitRexOpVarResolved(node: Rex.Op.Var.Resolved, ctx: ProblemCallback) =
        org.partiql.plan.Rex.Op.Var(node.ref)

    override fun visitRexOpVarUnresolved(node: Rex.Op.Var.Unresolved, ctx: ProblemCallback): PlanNode {
        error("Unresolved variable $node")
    }
    ...
    override fun visitRexOpCallStatic(node: Rex.Op.Call.Static, ctx: ProblemCallback) =
        org.partiql.plan.Rex.Op.Call.Static(fn = visitFn(node.fn, ctx), args = node.args.map { visitRex(it, ctx) })

    override fun visitRexOpCallDynamic(node: Rex.Op.Call.Dynamic, ctx: ProblemCallback) =
        org.partiql.plan.Rex.Op.Call.Dynamic(candidates = node.candidates.map {
            visitRexOpCallDynamicCandidate(
                it,
                ctx
            )
        },
    ...
    override fun visitRexOpCallDynamicCandidate(node: Rex.Op.Call.Dynamic.Candidate, ctx: ProblemCallback) =
        org.partiql.plan.Rex.Op.Call.Dynamic.Candidate(fn = visitFn(node.fn, ctx),
            coercions = node.coercions.map { it?.let { visitFn(it, ctx) } })
    ...
    override fun visitRexOpErr(node: Rex.Op.Err, ctx: ProblemCallback): PlanNode {
        error("Plan had error: $node")
    }
    ...
    override fun visitRelOpSort(node: Rel.Op.Sort, ctx: ProblemCallback) =
        org.partiql.plan.Rel.Op.Sort(input = visitRel(node.input, ctx),
    ...
    override fun visitRelOpErr(node: Rel.Op.Err, ctx: ProblemCallback): PlanNode {
        error("Plan had an error: $node")
    }
