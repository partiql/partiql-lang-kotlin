...
internal object RexConverter {
    ...
    private object ToRex : AstBaseVisitor<Rex, Env>() {
        ...
        override fun visitExprUnary(node: Expr.Unary, context: Env): Rex {
            ...
            val args = listOf(arg)
            // Fn
            val id = Identifier.delimited(node.op.name.lowercase())
                ...
                val path = when (step) {
                    ...
                    is Expr.Path.Step.Symbol -> {
                        when (step.symbol.caseSensitivity) {
                            org.partiql.ast.Identifier.CaseSensitivity.SENSITIVE -> {
                                // case-sensitive path step becomes a key lookup
                                rexOpPathKey(current, rexString(step.symbol.symbol))
                            }
                            org.partiql.ast.Identifier.CaseSensitivity.INSENSITIVE -> {
        ...
        override fun visitExprCase(node: Expr.Case, context: Env) = plan {
            ...
            val id = Identifier.delimited(Expr.Binary.Op.EQ.name.lowercase())
        ...
        override fun visitExprIsType(node: Expr.IsType, ctx: Env): Rex {
            ...
            var op = when (val type = node.type) {
                is Type.NullType -> Rex.Op.IsNull(value)
                is Type.Missing -> Rex.Op.IsMissing(value)
                else -> Rex.Op.IsType(value, SqlTypes.from(node.type).toCType())
            }
            if (node.not == true) {
                op = negate(op)
            }
        ...
        override fun visitExprCast(node: Expr.Cast, ctx: Env): Rex {
            val type = SqlTypes.from(node.asType).toCType()
        ...
        private fun negate(call: Rex.Op): Rex.Op.Call {
            ...
            val id = Identifier.delimited(name.lowercase())
            // wrap
