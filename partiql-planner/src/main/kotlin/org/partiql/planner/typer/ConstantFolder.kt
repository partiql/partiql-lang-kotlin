...
internal object ConstantFolder {
    ...
    private object ConstantFolderImpl : PlanRewriter<StaticType>() {
        ...
        private fun default(op: Rex.Op, type: StaticType): Rex = Plan.rex(type, op)
        ...
        override fun visitRexOpTupleUnion(node: Rex.Op.TupleUnion, ctx: StaticType): Rex {
            ...
            val fields = args.flatMap { arg ->
                val argType = arg.type as? StructType ?: return default(node, ctx)
                if (argType.contentClosed.not()) { return default(node, ctx) }
                argType.fields.map { it to arg }
            }.map { field ->
                val fieldName = Plan.rex(StaticType.STRING, Plan.rexOpLit(stringValue(field.first.key)))
                Plan.rexOpStructField(
                    k = fieldName,
                    v = Plan.rex(
                        field.first.value,
                        Plan.rexOpPath(field.second, steps = listOf(Plan.rexOpPathStepIndex(fieldName)))
                    )
                )
            }
            return fold(Plan.rex(ctx, Plan.rexOpStruct(fields)))
        }
        ...
        private fun visitRexOpCallEq(folded: Rex.Op.Call, ctx: StaticType): Rex {
            ...
            if (lhsFolded == rhsFolded) {
                return Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(true)))
            }
            ...
            return Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(lhsLiteral == rhsLiteral)))
        }
        ...
        private fun visitRexOpCallNot(folded: Rex.Op.Call, ctx: StaticType): Rex {
            ...
            return Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(boolean.not())))
        }
        ...
        private fun visitRexOpCallIsStruct(folded: Rex.Op.Call, ctx: StaticType): Rex {
            ...
            return when (val resultType = isStructLhs.type.flatten()) {
                is StructType -> Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(true)))
                ...
                is AnyOfType -> {
                    when {
                        ...
                        resultType.allTypes.all { it is StructType } -> Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(true)))
                        resultType.allTypes.any { it is StructType }.not() -> Plan.rex(StaticType.BOOL, Plan.rexOpLit(boolValue(false)))
